	...
	*/
	public init(data: NSData)
	{
		...
		tokens = 16.stride(through: numberOfTokens * 16, by: 16).map { parserOffset in
			...
			return SyntaxToken(type: stringForSourceKitUID(uid) ?? "unknown", offset: offset, length: length >> 1)
		}
	}
...
//		let commentTokensImmediatelyPrecedingOffset = tokensBeforeOffset.filterLastContiguous {
